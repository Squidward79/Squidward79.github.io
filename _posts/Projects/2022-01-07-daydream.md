...
말 그대로 서로 하고 싶었던 요소들을 집어넣어 게임을 만들기로 했습니다. 저는 기술적인 시도를 많이 하고싶었고 디자이너는 3D에 대한 시도를 해보고 싶어했습니다. 그렇게 아트에선 게임의 맵과 보스의 일러스트가 3D로 작업되었고, 프로그래밍 쪽에서는 `쉐이더`와 `개발툴 제작` 그리고 `디자인 패턴`의 실제 적용 등 그간 배웠거나 궁금했던 기술들을 구현하고자 하였습니다. 
