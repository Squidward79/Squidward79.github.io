...
<u>횡스크롤 액션게임</u>으로, 소원을 이루기 위해 탑에 올라 강적들을 쓰려뜨려야 하는 모험가들의 이야기를 담은 게임입니다. 2명의 캐릭터를 선택해 탑을 올라야 하며, 탑의 한층 한층에는 강력한 보스가 존재합니다. 다른 요소 없이 오로지 보스와의 전투에만 집중하는 **보스러시** 형식의 게임입니다. 현재까지는 2종류의 보스가 구현되어 있습니다.
...
개발 중 첫번째 보스를 만든 방법이 비효율적이라고 생각되어, 두번째 보스부터는 개발 기간을 단축시키기 위한 고민을 하여 패턴 개발 도구를 제작하여 개발을 이어나갔습니다.

첫번째 보스인 **지식의 골렘**은 각 부위별로 오브젝트를 만든 후 시간에 따른 부위별 위치를 애니메이션으로 직접 제작하여 `Animator`를 통해 움직이도록 만들었습니다.

 골렘의 양쪽 어꺠의 책장 색이 방향에 관게없이 항상 고정이었어야 했기 때문에 좌/우 방향 두가지 동일한 애니메이션을 만들었는데, 이 방식은 동작을 수정할 때마다 방향별로 전부 수정을 해주어야 해서 번거롭고 시간이 많이 소모되었습니다. Animator가 복잡해지는 것도 마음에 들지 않았습니다. 나중에서야 오히려 Bone을 적용하여 만들었다면 더 좋았을텐데 하는 아쉬움이 있습니다. 
...
골렘의 Ai는 스크립트 내에서 FSM과 각종 조건문에 따라서 행동을 수행하도록 했습니다.  
타겟과의 거리에 따라서 원거리 또는 근거리 공격 패턴을 사용한 후, 짧은 시간 타겟에게로 이동해 다시 공격을 시도합니다.

그러나 오로지 스크립트내부에서만 골렘의 행동을 제어할 수 있었고 조건문이 중첩되며 코드의 가독성이 떨어지는 문제와, 엔진 위에서는 공격패턴의 추가나 수정이 불가능한 문제로 상당히 불편하였습니다. 

또한 공격거리를 판단하는 AttackDistCheck 함수에서 공격패턴을 결정하고 있어 이름과 기능이 맞지 않는 좋지 않은 코드임을 확인할 수 있습니다.
...
두번째 보스인 **시간의 관리자** 는 오브젝트를 부위별로 조합하여 사용했던 첫 보스와 달리, 보스의 몸체를 최소한의 오브젝트만으로 구성했고 단일 스프라이트가 프레임에 따라 변하는 애니메이션 방식으로 제작방식을 변경하였습니다. 디자이너가 각종 행동의 프레임을 만들어내는라 조금 고생하기는 했지만..
...
패턴 생성기를 이용해 미리 정의되어있는 전체 동작들 중에 수행할 애니메이션을 직접 선택하고 수행하기 때문에 Animator가 transition으로 복잡해지는 문제가 해소되었고 동작의 추가 및 수정도 용이해졌습니다.  
<small>패턴 생성기에 대한 추가 설명은 하단에 있습니다</small>
...
패턴 생성기가 패턴을 선택하는 부분을 대신함으로서 첫번째 보스의 중첩조건문 등 복잡했던 패턴선택함수의 코드가 굉장히 간결해졌습니다. 이름과 기능이 맞지 않던 부분도 명확하게 분리되었습니다.
...
- 렌더링 순서 조정을 위한 쉐이더
  ...
  2D와 3D를 동시에 사용하다보니 기본 material로는 렌더링 순서에서 의도하지 않은 문제들이 발생하였습니다. 3D 파티클들이 2D 스프라이트에 가려진다거나 2D가 3D 배경 뒤에서 렌더링 된다거나 하는 문제가 있었습니다. 또한 같은 2D오브젝트 사이에서도 캐릭터와 보스 오브젝트의 z-fight문제 등 렌더링에 관한 이슈가 많았습니다. 이를 해결하기 위해 파티클 / 3D배경오브젝트 / 캐릭터 / 보스 등 종류별로 `Render Queue`를 조정한 material을 만들어 렌더링 순서를 의도한 대로 지정해주었습니다.
...
<p align="center">
  ...
  <img src="https://user-images.githubusercontent.com/45874696/150317297-a146eac1-36fc-423d-96ed-1e9df11b207c.png" alt="fourth" width="400"/>
