...
개발 중 첫번째 보스를 만든 방법이 비효율적이라고 생각되어, 두번째 보스부터는 개발 기간을 단축시키기 위한 고민을 하여 패턴 제작 도구를 제작하여 개발을 이어나갔습니다.
...
특히 몬스터 패턴 제작 도구를 만들어 본 것이 큰 경험이었습니다. 이전작인 The Fins에서 간단하게 만들었던 레벨 제작도구보다 더 체계적으로 구현했고, 그 과정에서 가중치확률 계산 등 수학적인 요소도 게임에 적용해 볼 수 있었습니다. 첫번째 보스를 만들며 답답했던 부분을 상당부분 개선하고 해결방법을 찾아냈다는 것에도 조금의 뿌듯함과 자신감을 얻었습니다.

액션게임을 만드려는 시도에서 액션을 만들어내는게 상당히 어렵다는 것을 깨달았습니다. 여러 레퍼런스를 참조하면서 몇번이나 캐릭터의 조작감을 개선하고, 테스트를 거치며 스킬 딜레이나 속도를 미세하게 조절하고, 타격감을 살리려면 어떤 방법을 써야할지 고민하는 등 액션이 있는 게임에 대한 고민을 많이 하게 되었던 프로젝트 입니다.
